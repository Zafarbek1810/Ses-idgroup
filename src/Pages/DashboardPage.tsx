...
const StatusBadge = ({ status }: { status: string }) => {
  const MAP: Record<string, string> = {
    ...
    "In Review": "bg-violet-50 text-violet-700 dark:bg-violet-950/40 dark:text-violet-400",
    Scheduled: "bg-cyan-50 text-cyan-700 dark:bg-cyan-950/40 dark:text-cyan-400",
    Rejected:  "bg-red-50 text-red-700 dark:bg-red-950/40 dark:text-red-400",
  };
  return (
    <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-[11px] font-medium ${MAP[status] ?? "bg-gray-50 text-gray-700"}`}>
      {status}
    </span>
  );
};
...
const StatCard = ({ label, value, description, icon: Icon, trend, iconBg, iconColor }: StatCardProps) => (
  <div className="bg-card rounded-2xl p-5 border border-border shadow-sm hover:shadow-lg hover:-translate-y-0.5 transition-all duration-200 group cursor-default">
    ...
    <div className="text-[28px] font-bold text-foreground leading-none mb-1.5">{value}</div>
      ...
      <div className="grid grid-cols-3 gap-4">
        ...
        <div className="col-span-2 bg-card rounded-2xl p-5 border border-border shadow-sm">
          <div className="flex items-center justify-between mb-5">
            <div>
              <h3 className="text-[14px] font-semibold text-foreground">Applications & Inspections Trend</h3>
          ...
          <ResponsiveContainer width="100%" height={195}>
            <AreaChart data={TREND_DATA} margin={{ top: 5, right: 0, bottom: 0, left: -18 }}>
              ...
              <Area type="monotone" dataKey="inspections" name="Inspections" stroke="#10B981" strokeWidth={2.5} fill="url(#gradInsp)" dot={false} />
            </AreaChart>
          </ResponsiveContainer>
        </div>
        ...
        <div className="bg-card rounded-2xl p-5 border border-border shadow-sm">
          <div className="mb-5">
            <h3 className="text-[14px] font-semibold text-foreground">Inspections by Region</h3>
          ...
          <ResponsiveContainer width="100%" height={195}>
            <BarChart data={REGION_DATA} layout="vertical" margin={{ top: 0, right: 8, bottom: 0, left: -8 }}>
              ...
              <Tooltip contentStyle={customTooltipStyle} />
              <Bar dataKey="count" name="Inspections" fill={primaryColor} radius={[0, 5, 5, 0]} maxBarSize={20} />
            </BarChart>
          </ResponsiveContainer>
        </div>
      </div>
      ...
      <div className="grid grid-cols-3 gap-4">
        ...
        <div className="col-span-2 bg-card rounded-2xl border border-border shadow-sm overflow-hidden">
          <div className="flex items-center justify-between px-5 py-4 border-b border-border">
            <div>
              <h3 className="text-[14px] font-semibold text-foreground">Recent Activities</h3>
              <p className="text-xs text-muted-foreground">Latest applications and inspections</p>
            </div>
            <div className="flex gap-1.5">
              <button className="p-1.5 rounded-lg hover:bg-secondary transition-colors text-muted-foreground" title="Filter">
                <Filter className="w-4 h-4" />
              </button>
              <button className="p-1.5 rounded-lg hover:bg-secondary transition-colors text-muted-foreground" title="Refresh">
                <RefreshCw className="w-4 h-4" />
              </button>
              <button className="p-1.5 rounded-lg hover:bg-secondary transition-colors text-muted-foreground" title="Export">
                <Download className="w-4 h-4" />
              </button>
            </div>
          </div>
              ...
              <thead>
                <tr className="border-b border-border">
                  {["ID", "Type", "Organization", "Inspector", "Status", "Date"].map(h => (
                    <th key={h} className="text-left text-[10px] font-bold text-muted-foreground uppercase tracking-wider px-5 py-3">
                      {h}
                    </th>
                  ))}
                </tr>
        ...
        <div className="space-y-4">
          ...
          <div className="bg-card rounded-2xl p-5 border border-border shadow-sm">
            ...
            <div className="grid grid-cols-2 gap-2">
              {[
                ...
                { icon: ClipboardCheck, label: "Start Inspection", color: "#10B981" },
                { icon: Award, label: "Issue Certificate", color: "#8B5CF6" },
                { icon: Download, label: "Export Report", color: "#F59E0B" },
                { icon: Calendar, label: "Schedule Visit", color: "#EC4899" },
                { icon: Archive, label: "View Archive", color: "#64748B" },
              ].map(action => (
                <button
                  key={action.label}
                  className="flex flex-col items-center gap-2 p-3 rounded-xl border border-border hover:shadow-sm transition-all group text-center"
          ...
          {/* Announcements */}
          <div className="bg-card rounded-2xl p-5 border border-border shadow-sm">
            <h3 className="text-[14px] font-semibold text-foreground mb-4">Announcements</h3>
            <div className="space-y-3">
              {ANNOUNCEMENTS.map(a => {
                const styles = {
                  info:    { bg: "rgba(14,165,233,0.07)", border: "rgba(14,165,233,0.2)", icon: <Info className="w-3 h-3 text-sky-500" />, dot: "#0EA5E9" },
                  warning: { bg: "rgba(245,158,11,0.07)", border: "rgba(245,158,11,0.2)", icon: <AlertCircle className="w-3 h-3 text-amber-500" />, dot: "#F59E0B" },
                  success: { bg: "rgba(16,185,129,0.07)", border: "rgba(16,185,129,0.2)", icon: <CheckCircle className="w-3 h-3 text-emerald-500" />, dot: "#10B981" },
                ...
                return (
                  ...
                  >
                    <div className="flex items-start gap-2.5">
                      <div className="w-5 h-5 rounded-full flex items-center justify-center shrink-0 mt-0.5" style={{ background: `${styles.dot}20` }}>
                        {styles.icon}
                      </div>
